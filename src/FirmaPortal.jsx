...
function Login(){
 ...
 return <main style={{minHeight:'100vh',background:bg,display:'grid',placeItems:'center',fontFamily:'Arial,sans-serif',padding:20}}><div style={{width:'min(430px,100%)',background:'#fff',border:`1px solid ${border}`,borderRadius:16,padding:24}}><div style={{textAlign:'center'}}><img src="/logo-asociacion-comercio-justo.png" alt="" style={{width:150,maxHeight:120,objectFit:'contain'}}/><h2 style={{color:green,marginBottom:3}}>Portal de Firmas NODO</h2><div style={{fontSize:12,color:muted}}>Asociación de Comercio Justo Campos Bórquez A.C.</div></div><label style={{display:'block',fontSize:12,fontWeight:800,color:green,marginTop:20}}>Usuario</label><input style={{...input,margin:'5px 0 12px'}} autoCapitalize="none" autoCorrect="off" value={username} onChange={e=>setUsername(e.target.value.toLowerCase().replace(/\s+/g,''))}/><label style={{display:'block',fontSize:12,fontWeight:800,color:green}}>Contraseña de acceso</label><div style={{position:'relative',marginTop:5}}><input style={{...input,paddingRight:85}} type={show?'text':'password'} value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==='Enter'&&go()}/><button type="button" onClick={()=>setShow(v=>!v)} style={{position:'absolute',right:7,top:'50%',transform:'translateY(-50%)',border:0,background:'transparent',color:green,fontWeight:800}}>{show?'Ocultar':'Mostrar'}</button></div><button style={{...btn(),width:'100%',marginTop:14}} disabled={busy} onClick={go}>{busy?'Ingresando…':'Ingresar a mis documentos'}</button>{msg&&<div style={{marginTop:12,padding:10,borderRadius:8,background:'#fff0f0',color:'#912727',fontSize:12}}>{msg}</div>}<p style={{fontSize:10,color:muted,marginTop:18}}>Tu usuario y contraseña identifican tu cuenta de firmante. Una vez dentro, puedes revisar y firmar los documentos que te hayan sido asignados.</p></div></main>
}
